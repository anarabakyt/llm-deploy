import type { Model, Chat, Message, User, ModelResponse } from '../entities';
...
class ApiService {
  ...
  async sendMessageToAllModels(text: string): Promise<{ userMessage: Message; modelResponses: ModelResponse[] }> {
    const response = await this.post<{ userMessage: Message; modelResponses: ModelResponse[] }>('/messages/broadcast', { text });
    return response.data;
  }
